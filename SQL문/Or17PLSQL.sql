...
-- 예제2] 일반변수 및 info
/*
시나리오] 사원 테이블에서 사원번호가 120인 사원의 이름과 연락처를 출력하는
    PL/SQL 문을 작성하시오.
*/
...
declare
    ...
    => 변수명 자료형(큭;)
    단, 기존의 생성된 컬럼의 타입과 크기를 참조하여 선언해주는 것이 좋다.
...
begin
    /*
    실행부 : select절에서 가져온 결과를선언부에서 선언한 변수에
...
시나리오] 부서번호 10인 사원의 사원번호, 급여, 부서번호를 가져와서
    아래 변수에 대입 후 화명상에 출력하는 PL/SQL문을 작성하시오.
    단, 변수는 기존 테이블의 자료형을 참조하는 '참조변수'로 선엉하시오.
*/
...
declare
    -- 사원 테이블의 특정 컬럼의 타입과 크기를 그대로 참조하는 변수로 선언한다.
...
-- 예제4] 참조변수2 (전체컬럼을 참조)
/*
시나리오] 사원번호가 100인 사원의 레코드를 가져와서 emp_row 변수에 전체 컬럼을
    저장한 후 화면에 다음 정보를 출력하시오.
    단, emp_row는 사원 테이블이 전체컬럼의 저장할 수 있는 참조변수로 선언해야한다.
    출력정보 : 사원번호, 이름, 이메일, 급여
*/
declare
    /*
...
/*
복합변수
    : class를 정의하듯 핖ㄹ요한 자료형을 묶어 하나의 자료형을 만든 후
    생성하는 변수는 말한다.
    형식]
    ...
    복합변수 자료형을 만을 때는 일반변수화 참조변수 2가지를 복합해서
    사용할 수 있다.
*/
/*
...
위에서 생성한 자료형을 이용하여 복합변수 rec2를 생성후 사원번호 100번의
정보를 할당한다.
2.1의 내용을 출력한다.
...
/*
바인드 변수
    : 호스트 환경 선언된 변수로써 비 PL/SQL 변수이다.
    호스트 환경이란 PL/SQL의 블럭을 제외한 나머지 부분을 말한다.
    콘솔(CMD)에서는 SQL>명령 프롬포트가 있는 상태를 말한다.
...
-- PL/SQL 작성
declare
    -- 선언부에는 이와같이 아무내용 없을 수도 있다.
begin
    -- 바이든 변수는 일반변수와의 구분을 위해 :(콜론)을 추가해야 한다.
    :return_var := 999;
...
-- 퀴즈] 위 예제를 치환연산자를 통해 숫자를 입력 받을 수 있도록 수정하시오.
declare
    -- 선언부에서 숫자타입의 변수를 선언
    num number;
begin
...
begin
    ...
    elsif로 기술해야 한다. 또한, 중괄호 대신 then과 end if;가 사용횐다.
    */
    if emp_dept = 50 then
        dept_name := 'Shipping';
...
begin
    ...
    /*
    case~when문이 if문과 다른점은 할당할 변수를 먼저 선언한 후 문장 내에서
...
/*
반복문1 : Basic loop
    Java의 do~while문과 같이 조건체크 없이 loop진입한 후 탈출 조건이 
    될때까지 반복한다. 탈출 시에는 exit를 사용한다.
*/
...
/*
시나리오] Basic loop문으로 1부터 10까지의 정수의 합을 구하는 프로그램을 
작성하시오.
*/
declare
    -- 반복문에서 증가시킬 변수
...
/*
반복문1 : while문
    Basic loop와는 다르게 조건을 먼저 실행한 후 실행한다.
...
커서(Cursor)
    ...
    선언방법]
        Cursor 커서명 Is
            Select 뭐리문. 단 into절이 없는 형태로 기술한다.
            
        Open Cursor
            : 쿼리를 수행하라는 의미. 즉 Open 할 때 Cursor 선언시의 select 문장이
            실행되어 결과셋을 얻게된다. Curesor는 그 결과셋의 척번째 행에
            위치하게 된다.
        Fetch~Into~
...
declare
    ...
    /*
    커서선언 : 부서 테이블의 모든 레코드를 조회하는 select문으로 into절이
        없는 형태로 기술한다.
        쿼리의 실행결과 cur1의 변수에 저장된다.
    */
    cursor cur1 is
        select
...
begin
    ...
    */
    ...
    loop
        ...
        -- 탈출 조건으로 더이상 인츨할 행이 없으면 exit가 실행된다.
...
1. 연관배열(Associative Array)
    : Key와 Value의 한 쌍으로 구성된 컬랙션으로 Java의 해쉬맵과 같은 개념이다.
...
begin
    /*
    커서를 오픈하여 쿼리문을 실행한 후 얻어온 결과셋의 객수만큼 반복하여
    사원명을 인출한다.
    */
    open emp_cur;
    loop
...
--예제21] VArray(Variable Array)
    : 고정길이를 가진 배열로써 일반 프로래민 언어에서 사용하는 배열과
...
declare
    ...
    -- VAarrya형 배열변수 선언
    v_arr vaType;
    -- 인덱스로 사용할 변수 선언 및 초기화
    cnt number := 0;
begin
    -- 생성자를 통한 값의 초기화 총 5개 중 3개만 할당한다.
...
declare
    -- VAarry 자료형 선언. 배열에 저장될 값은 사원아이디 컬럼을 참조하여 생성
    type vaType1 is
...
3. 중첩 테이블(Nested table)
    : Varry와 비슷한 구조의 배열로써 배열의 크기를 명시하지 않으므로
...
declare
    ...
    nt_array ntType;
begin
    -- 생성사를 통해 값을 할당한다. 이대 크기 4인 중첩테이블이 생성된다.
    ...
    -- 에러발생. 첨자가 개수를 넘었습니다.(자동으로 확장되지 않는다.)
--  nt_array(5) := '다섯번째값??할당??';
...
/*
시나리오] 중첩테이블과 for문을 통해 사원테이블의 전체 레코드의 사원번호와 
이름을 출력하시오.
*/
declare
    /*
    중첩테이블 자료형 선언 및 변수선언 : 사원테이블 전체 컬럼을 참조하는 
        참조변수의 형태이므로 하나의 레코드(Row)를 저장할 수 있는 존재로 선언
    */
    type ntType is
        table of employees%rowtype;
    nt_array ntType;
begin
    -- 크기를 지정하지 않은 상태로 생성자를 통해 중첩테이블을 초기화 한다.
    nt_array := ntType();
    
    /*
    사원 테이블의 레코드 수 만큼 반복하면서 레코드를 하나씩 변수 rec에
    ...
    */
    for rec in (select * from employees order by employee_id) loop
        ...
        nt_array.extend;
        -- 마지막 인덱스 값을 얻어온 후 사원정보 레코드를 삽입한다.
