...
시퀀스 & 인덱스 
설명 : 테이블의 기본키 필드에 순차적으로 일련번호를 부여하는 시퀀스와
    검색속도를 향상시킬 수 있는 인덱스
****************/
-- study 계정으로 하기
...
create sequence 시퀀스명
    ...
    [Maxvalue m | NoMaxvalue]   -> 시퀀스 최대값 지정 : 디폴트 1.0000E+28
...
주의사항
    1. start with에 minvalue보다 작은 값을 지정할 수 없다. 즉, start with 값은
    minvalue와 같거나 커야한다.
...
/*
시퀀스 생성 후 최초실행 시 오류가 발생한다. nextval을 먼저 실행한 후
실행해야 문제없이 출력된다.
*/
select seq_serial_num.currval from dual;
/*
다음 입력할 시퀀스(일련번호)를 반환한다. 실행할 때마다 다음으로 넘어간다.
*/
...
insert into tb_goods values (seq_serial_num.nextval, '꿀꽈배12');
/*
...
인덱스(Indec)
    ...
    - 인덱스는 명시적(create index)과 자동적(primary key, uniqe)
    으로 생성할 수 있다.
