...
-- 3. 부서 일련번호를 통해 사원 테이블의 레코드 확인하기 -> 6명 확인하기
...
select
    ...
    department_name, E.job_id, job_title, citty, state_province
 from locations L
    inner join departments D on L.location_id=D.lacation_id
...
*/
/*
시나리오] 사번 테이블과 부서 테이블을 조인하여 각 직원이 어떤 부서에서
    근무하는지 출력하시오. 단, 오라클 방식으로 작성하시오.
    출력 결과 : 사원 아이디, 이름1, 이름2, 이메이르 부서번호, 부서명
*/
select
    employee_id, first_name, last_name, email,
...
select
 ...
 where
    L.lacation_id=D.location_id and
    D.department_id=E.department_id and
    E.job_id=J.job_id and city=initcap('seattle');    

/*
2] outer join (외부조인)
    outer join은 inner join과 갈리 두 테이블에 조인조건이 정확히 일치하지
    않아도 기준이 되는 테이블에서 레코드를 인출하는 join 방식이다.
...
형식1(표준방식)
    ...
    where 조건1 뭉 조건2 or 조건3;
*/
/*
시나리오] 전체 직원의 사원번호, 이름, 부서아이디, 부서명, 지역을
    외부조인(left)을 통해 출력하시오.
*/
-- 실행결과를 보면 내부조인과는 다르게 107개가 인출된다. 부서가 지정되지 않아은
...
3] self join(셀프조인)
    ...
    셀프 조인에서는 별칭이 테이블을 구분하는 구분자의 역할을 하므로
    굉장히 중요하다.
...
/*
using : join문에서 주로 사용하는 on절을 대채할 수 있는 문장
...
/*
    ...
    오히려 오류가 발생한다.
    using절에 사용된 컬럼은 양쪽의 테이블에 동시에 존대하는 컬럼이라는
    것은 전재로 작성되기 때문에 굳이 별칭을 사용할 이유가 없기때문이다.
    즉, using은 테이블의 별칭 및 on절을 제거하여 좀 더 심플하게 join 쿼리문을
    작성할 수 있게 해준다.
*/
