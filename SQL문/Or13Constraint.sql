...
primary key : 기본키
    ...
    - 주로 레코드 하나를 특정하기 위해 상용된다.
*/
/*
형식1] 인라인 방식 : 컬럼 생성 시 우측에 제약조건을 기술한다.
    create table 테이블명(
        컬럼명 자요형(크기) [constraint 제약명] primary key
    );
    * []대괄호 부분은 생략 가능하고, 생략 시 제약명을 시스템이 자동으로
    부여한다.
*/
...
/*
제약조건 및 목록 확인하기
    tab : 현재 계정에 생성된 테이블이 목록을 확인할 수 있다.
...
desc tb_primary2;
select * from user_cons_columns; -- 간략하게
select * from user_constraints; -- 자세하게
...
/*
형식2] 테이블을생성한 후 alter문으로 제역조건 추가
...
-- PK는 테이블당 하나만 생성할 수 있다. 따라서 헤당 문장은 에러가 발생한다.
...
insert into tn_unique (idx, name, telephone, nickname)
...
/*
    ...
    컬럼이 동시에 동일한 값을 가지는 걍우가 아니라면 중복된 값이 허용된다.
...
create table tb_foreign1 (
    ...
    f_name varchar2(50),
    /*
        자식 테이블인 tb_foreign1에서 부모테이블인 tb_primary2의 user_id컬럼을
        참조하는 외래키를 생성한다.
    */
...
-- 입력 성공. 부모 테이블에 white라는 아이디가 있음.
insert into tb_foreign1 values(2, '하얀색', 'white');
/*
    자식 테이블에서 참조하는 [코드가 있으므로, 부모테이블의 레코드를 삭제할 수
    없다. 이 경우 반드시 자식 테이블의 레코드를 먼저 삭제한 후부모테이블의
    레코드를 삭제해야 한다.
*/
-- 오류 발생
...
/*
    ...
    부모 테이블의 참조할 레코드가 없으면 자식 테이블에 insert할 수 없다.
    자식 테이블의 부모를 참조하는 레코드가 남아있으면 부모 테이블의 
    레코드를 delete할 수 없다.
*/
/*
형식2] 아웃라인 방식
...
[on delete cascade]
    ...
    형식]
        컬럼명 자료형 references 부모 테이블 (pk컬럼)
            on delete cascade;
[on delete set null]
    : 부모 레코드 삭제시 자식 레코드 값이 null로 변경된다.
    형식]
        컬럼명 자료형 references 부모테이블 (pk컬럼)
            on delete set null
            
* 실무에서 스팸게시물을 남긴 회원만 그 게시글을 일괄적으로 삭제해야할 때
    사용할 수 있는 옵션이다. 단, 자식 테이블의 모든 레코드가 삭제되므로 사용에
    주의해야 한다.
*/
create table tb_primary4(
    user_id varchar2(30) primary key,
...
/*
    외래키가 설정된 겨우 반드시 부모테이블에 레코드를 먼저 입력한 후
    자식 테이블에 입력해야 한다.
*/
...
/*
    ...
    참조기 부분만 null 값으로 변경된다. 따라서 더이상 참조할 수 없는 레코드로
    변경된다.
*/
...
create table tb_not_null (
    m_idx number(10) primary key,   -- PK 이므로 NN
...
insert into tb_default (id) values ('bbbb');-- 컬럼 자체가 없으므로 default 값 입력
