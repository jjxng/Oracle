...
/*
[사용자 계정 생성 및 권한설정]
해당 부분은 DBA권한이 있는 최고 관리자(sys, system)으로 접속한 후
실행해야 한다.
새로운 사용자 계정이 생성된 후 접속 및 쿼리실행 테스트는 CMD(명령 프롬포트)
에서 진행한다.
*/
...
4] Role(롤, 역할)을 통한 여러가지 권한을 동시에 부여하기
    ...
    * 우리는 실습을 위해 새롭게 생성한 계정에 connet, resource롤을 주로
    부여한다.
*/
...
-- test_user1로는 접속할 수 없다. 접속 시 오류가 발생한다.

/*
6] 사용자 계정 삭제
    형식] drop user 사용자계정 [cascade];
    * cascade를 명시하면 사용자 계정과 관련된 모든 데어터베이스 스키마가
...
select * from dba_users where lower(username)='test_user1'; -- 결과 없으
...
/*
테이블 스페이스란?
    디스크 공간을 소비하는 테이블과 뷰, 그리고 그 밖의 다른 데이터베아스
...
/*
    ...
    2m 의 용량을 할당한다.
*/
-- cmd에서 테이블에 insert 되는지 확인해본다.

-- 2번째 사용자 추거 : 테이블 스페이스 users로 지정하여 생성한다.
...
-- 앞에서 지정한대로 테이블 스페이스 users를 사용하고 있다.
select username, default_tablespace from dba_users
...
이미 생성된 사용자의 
*/
alter user test_user1 defaut tablespace users;